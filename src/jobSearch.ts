...
let jobsToShow: any[] = [];
let keywordComponent: import("@yaireo/tagify") <TagData>, resultElement: Element | null, main;

export function execute() {
    const popup = createPopup(findButton);
    keywordComponent = popup.keywordComponent;

    resultElement = document.querySelector('[data-testid="result-total-text"]');
...
async function findButton() {
    ...
    const keywords = keywordComponent.value?.map((x: { value: any; }) => x.value);
